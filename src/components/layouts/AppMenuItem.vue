  ...
  <li class="menu-item" :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActive, 'child-active': hasActiveItem }">
    ...
    <Transition v-if="item.children && item.visible !== false" name="layout-submenu">
      <ul v-show="root ? true : isActive" class="layout-submenu">
        <AppMenuItem v-for="child in item.children" :key="child.label + '_' + (child.to || child.pathRef)" :item="child" :root="false" :parentPath="fullPath" />
      </ul>
    </Transition>
  </li>
</template>
...
const fullPath = computed(() => (props.item.pathRef ? (props.parentPath ? props.parentPath + props.item.pathRef : props.item.pathRef) : null))
...
function itemClick(event, item) {
  ...
  if (item.children) {
    if (isActive.value) {
      layoutState.activePath = layoutState.activePath.replace(item.pathRef || item.to, '')
    } else {
