<template>
  <aside class="flex flex-col h-full fixed top-0 left-0 shadow-sidenav overflow-hidden max-w-[320px]">
    <nav
      class="vas-menu"
      ref="sidebarmen"
      :class="sidebarClass"
    ...
    >
      ...
      >
        ...
        <template v-for="(item, index) in computedMenuList" :key="index">
          <MenuItem v-if="!item?.header && !item?.LineShow" :item="item" :depth="0" :smallMenu="smallMenu" :siblingsHaveIconProp="siblingsHaveIcon" />
      ...
      <div v-if="BottomMiniMenuBtn" class="bottomBtn" @click="toggleMiniCollapse">
        <div v-if="!$slots?.BottomMiniMenuBtn" class="icons bottomBtnIcon" :class="{ ssdSpin: !miniMenuRef }"></div>
...
export default {
  ...
  setup(props, context) {
    const { getIsCollapsed: isCollapsed, getIsminiMenu: miniMenuRef, updateMenuScroll, updateMenuHover, updateminiMenu, menuDirection, updateIsCollapsed, addIdToMenuItems } = initAwsomeSideBar(props, context)
