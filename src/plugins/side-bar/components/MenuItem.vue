<template>
  ...
  >
    ...
    >
      <div class="left" ref="labelRef" :class="{ marginAuto: miniMenu && depth === 0, collapseEnd: miniMenu }">
        <template v-if="!removeIconSpace || (removeIconSpace && siblingsHaveIconProp)">
          ...
          <component v-else-if="itemPrepandIcon" :is="itemPrepandIcon" :icon="item?.icon" :active="active" :miniActive="miniActive" :isChildrenMenuOpen="showChildren"></component>
        </template>
      ...
      <template v-if="(miniMenu && depth != 0) || !miniMenu">
        ...
        <div v-if="item.children && itemApendIcon">
          <component v-if="itemApendIcon" :is="itemApendIcon" :icon="item?.icon" :isChildrenMenuOpen="showChildren" :active="active" :miniActive="miniActive"> </component>
        </div>
      </template>
    </div>
    ...
    <div v-if="!miniMenu || (depth != 0 && miniMenu)">
      <div class="items-container" :class="{ 'small-menu': smallMenu }" :style="{ maxHeight: heifOfContainer, transition: transitionTime }" ref="container" v-if="item.children">
        <template v-if="renderChildren">
          <MenuItem v-for="(item, index) in item.children" :siblingsHaveIconProp="siblingsHaveIcon" :isParentFlat="siblingsHaveIconProp" :key="index" :item="item" :depth="depth + 1" :smallMenu="smallMenu" />
        </template>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          <component v-else :labelName="item?.name" :active="active" :miniActive="miniActive" :isChildrenMenuOpen="showChildren" :is="menuItemLabel" />
        </div>
      </div>
      ...
      >
        <template v-if="renderChildren">
          <MenuItem v-for="(item, index) in item.children" :siblingsHaveIconProp="siblingsHaveIcon" :isParentFlat="siblingsHaveIconProp" :key="index" :item="item" :depth="depth + 1" :smallMenu="smallMenu" :setMaxHeightTopCProp="setMaxHeightTopC" :isMakeSpace="MakeSpace" />
        </template>
      </div>
      <div class="labelminiSub" v-if="depth == 0 && MakeSpace"></div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const { animationDuration, menuType, widthMiniMenu, childrenOpenAnimation, removeIconSpace, vueRouterEnabel, keepChildrenOpen, checkButtonActive, ChildrenOpenActiveRoute, collapsed, closeOpenMenuOnHrefPush, position, keepOneMenuOpenAtAtime } = inject('sidebarProps')
